...
iWant:
  ...
  de: "eine gestellte Rechnung stornieren, sodass sie unmissverständlich als ungültig gekennzeichnet ist"
soThat:
...
status: done
aiContribution:
  en: "The AI proposed taking a snapshot of the invoice on cancellation, mirroring what the offer side does when an offer is superseded. It turned out to be unnecessary and was dropped: an exported invoice was already frozen by the export, so a cancelled invoice needs nothing frozen at cancellation time. The transition writes the state and the timestamp and nothing else, which is what makes it safe to reason about. The split of cancellation into two stories by VAT status is mine and is legal, not technical."
  de: "Die KI schlug vor, beim Stornieren einen Snapshot der Rechnung zu nehmen, analog zur Angebotsseite beim Ersetzen eines Angebots. Das erwies sich als unnötig und wurde verworfen: Eine exportierte Rechnung wurde bereits durch den Export eingefroren, eine stornierte Rechnung braucht zum Stornozeitpunkt also nichts Eingefrorenes. Der Übergang schreibt Status und Zeitstempel und sonst nichts, und genau das macht ihn nachvollziehbar. Die Aufteilung des Stornierens in zwei Stories nach Umsatzsteuerstatus ist meine Entscheidung und eine juristische, keine technische."
